...
def read_RSimages(mode):
    ...
    dsm_path = os.path.join(root, mode, 'dsm')
    mask_path = os.path.join(root, 'groundtruth_noBoundary') #'groundtruth'
    data_list = os.listdir(img_path)
    data, labels = [], []
    count=0
    for it in data_list:
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=40, random_flip = False, sliding_crop=False, crop_size=640/8, padding=False):
        self.crop_size = crop_size
        self.crop_nums = crop_nums
        self.random_flip = random_flip
        self.random_crop = random_crop
        data, labels = read_RSimages(mode)
        if sliding_crop:
